...
class ItemUpdateApi {
  ...
  ///   The item id.
  ///
  /// * [BaseItemDto] baseItemDto (required):
  ///   The new item properties.
  Future<Response> updateItemWithHttpInfo(String itemId, BaseItemDto baseItemDto,) async {
    // ignore: prefer_const_declarations
  ...
  /// * [BaseItemDto] baseItemDto (required):
  ///   The new item properties.
  Future<void> updateItem(String itemId, BaseItemDto baseItemDto,) async {
    final response = await updateItemWithHttpInfo(itemId, baseItemDto,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
